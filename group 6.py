...
def apply_color_to_image():
    global current_image, colored_image, drawing_layer, draw, current_angle
    color = colorchooser.askcolor()[1]
    if color and current_image:
...
def open_camera():
    ...
    if ret:
        ...
        # Lưu hình ảnh vào tệp tin đã chọn
        save_path = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG files", "*.png"), ("All files", "*.*")])
        if save_path:
...
buttons.append(tk.Button(canvas, text="Color to draw", command=choose_color))
...
buttons.append(tk.Button(canvas, text="open camera", command=open_camera))
